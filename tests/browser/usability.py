...
URL=os.environ.get("DEMO_URL", "file:///mnt/user-data/outputs/usc-ula-demo.html")
errs=[]; ok=[]
...
with sync_playwright() as p:
    ...
    if 'saving' in h1(pg).lower(): good(f"Email asked only after the recommendation: '{h1(pg)}'")
    else: bad(f"Expected the save screen, got '{h1(pg)}'")
    if pg.locator('.stepper').count()==0: good("and it draws no checkout stepper — saving is not a step of buying")
    else: bad("the save screen still shows the checkout stepper")
    if pg.locator('.savepanel__recap .answer-chip').count()>=3: good("the answers being saved are shown before the email is asked for")
    ...
    if pg.locator('#resume-link').count(): good("and the resume link is right there")
    else: bad("no resume link after saving")

    # 11b. and the other flow: the plan CTA goes straight to the details form
    pg.locator('[data-go="recommendation"]').first.click(); pg.wait_for_timeout(600)
    pg.locator('.planbox__cta button:visible, .paybar button:visible').first.click(force=True); pg.wait_for_timeout(700)
    if 'set up your membership' in h1(pg).lower(): good(f"'Continue with…' goes straight to the details form: '{h1(pg)}'")
